...
<div class="prereq">
<p><strong>Prerequisites:</strong> the whole course. Concretely, you should be comfortable with the pieces you are about to wire together: <a href="../module-04/lesson-02.md">byte-level BPE (04.2)</a>, <a href="../module-06/lesson-01.md">assembling GPT-2 (06.1)</a>, <a href="../module-07/lesson-01.md">the pretraining loop (07.1)</a>, <a href="../module-14/lesson-02.md">SFT loss masking (14.2)</a>, <a href="../module-16/lesson-02.md">RLVR (16.2)</a>, and <a href="../module-19/lesson-02.md">experiment design (19.2)</a>. See the <a href="../../curriculum/course-outline.md">curriculum map</a> for the full dependency graph.</p>
...
- **Look at:** how `merges` (an ordered dict of `(a, b) -> new_id`) is applied *in learned order* inside `encode`. That ordering is the whole subtlety. See [04.2 · Byte-level BPE](../module-04/lesson-02.md).
...
- **Look at:** weight tying (`transformer.wte.weight is lm_head.weight`) and the scaled residual init `0.02 / sqrt(2 * n_layer)`. See [06.1 · Assembling GPT-2](../module-06/lesson-01.md).
...
- **Look at:** the six-line inner step in `train` and how gradient accumulation makes a large effective batch fit in small memory. See [07.1 · The pretraining loop](../module-07/lesson-01.md).
...
- **Look at:** why the target is just the input shifted left by one — that single shift is the entire supervision signal. See [04.3 · The data pipeline](../module-04/lesson-03.md) and [Module 11 · Data engineering](../module-11/lesson-01.md).
...
- **Look at:** the roofline / memory-hierarchy story and mixed precision. See [Module 08 · GPU performance](../module-08/lesson-01.md).
...
- **Look at:** the power-law fit and the 6ND compute bookkeeping — a bigger model reaches a lower loss for the same steps, but costs more per step. See [Module 10 · Scaling laws](../module-10/lesson-01.md).
...
- **Look at:** the token-weighted mean cross-entropy in `evaluate_perplexity`, and length-normalized log-likelihood in `multiple_choice_score`. See [Module 13 · Evaluation](../module-13/lesson-01.md).
...
- **Look at:** why the assistant's `<|end|>` closer *is* a response token (so the model learns to stop). See [14.2 · Loss masking for SFT](../module-14/lesson-02.md).
...
- **Look at:** how the trainable-parameter count collapses to `r·(in + out)`. See [14.3 · LoRA](../module-14/lesson-03.md).
...
- **Look at:** how the prompt-only `beta·log Z(x)` term cancels in the difference, leaving a loss with only the policy and a frozen reference. See [15.3 · DPO](../module-15/lesson-03.md).
...
- **Look at:** the Bradley-Terry link $P(y_w \succ y_l) = \sigma(r(x,y_w) - r(x,y_l))$ and where the KL penalty enters the reward. See [15.1–15.2 · RLHF](../module-15/lesson-01.md).
...
- **Look at:** the correct-rate rising in `run_rlvr`'s history. See [16.1 · GRPO](../module-16/lesson-01.md) and [16.2 · RLVR](../module-16/lesson-02.md).
...
- **Look at:** the deterministic majority-vote tie-break, and `mini_r1`'s SFT-then-RL two-stage loop. See [Module 17 · Reasoning models](../module-17/lesson-01.md).
...
- **Look at:** the AST-vetted `calculator` (an allow-list, not a block-list). See [18.1 · Tool schemas](../module-18/lesson-01.md).
...
- **Look at:** how `run_react` is tool-registry-agnostic — tools are *injected*, never imported. See [19.1 · ReAct → a mini SWE-agent](../module-19/lesson-01.md).
...
- **Look at:** the VARIABLES section of the report — "one independent variable, everything else a control." See [19.2 · Experiment design](../module-19/lesson-02.md).
